...
**NEXT-002（Phase 2-2 まで完了・未コミット）。** 残りは Supabase 設定（2-3）待ち。
詳細は下の「NEXT-002」。
...
| Phase | 内容 | 状態 |
| --- | --- | --- |
| 2-0 | 報告された2症状の**再現確認** | ✅ 本人の実機で実施。症状1（PCでホームへ戻る）を再現・原因確定 |
| 2-1 | 統合の安全性 | ✅ 完了（未コミット） |
| 2-2 | 診断とエラーの言語化 / OAuth復帰の修正 | ✅ 今回完了（未コミット） |
| 2-3 | Storage の利用者分離と RLS | 🔑 **Supabase 設定が要る** |
