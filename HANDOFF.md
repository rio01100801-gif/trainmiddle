...
**配信済み（`forge-v45`・コミット`c49c647`）。**

---

## 未コミット変更

**あり（2026-08-01・FITのラップ分類の不具合を修正）。**

利用者から報告のあった「fit読み込み自体はできたけどメニュー内容の読み取りが
うまくいってない」の原因を特定して直した。`classifyLaps` が疾走群と休息群を
**中央値**で切っていたため、W-up/C-downのlapが無いFIT（時計で「本」と
「休み」だけラップを押した場合）で `300m × 4` が `300m × 1` になっていた。
ソートしたペース列の**最大の隔たり**で切る方式に変更し、混合比に左右されない
ようにした。副次的に、ビルドアップ走が実在しない「1km × 1」として
記録される問題も直った（今は判定できないと正直に返す）。
根拠はREADME「なぜ中央値をやめたか（2026-08-01）」。

**前回（forge-v45・2026-08-01・コミット`c49c647`・配信済み）**:
リファレンス画像13点に基づくUI全面刷新（8フェーズ）。詳細はREADME.mdの
「リファレンスUI刷新（forge-v45 / 2026-08-01）」を参照。

**前回（forge-v43・2026-07-31・配信済み）**: 利用者からの報告7件のうち、
情報不足の2件（FIT読み取り精度の具体例待ち／「文字が読みにくい」の
対象画面待ち）を除いた5件を実装。※FIT読み取り精度はこの版で
再現に成功し、上記のとおり修正済み。要約:
