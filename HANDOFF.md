...
| --- | --- |
| branch | `main` |
| HEAD | `ca23f3a`（`feat: 疲労兆候による低リスク候補優先を既存生成ロジックに追加`。forge-v41としてgh-pages配信済み） |
| `origin/main` との差 | **0 / 0**（完全一致）。ただし本セッションの分析画面改善（下記）が**未コミット** |
...
**あり（2026-07-31・「分析・データ可視化機能の再設計」の大規模提案を検討し、
小さな代替案2件を実装。これから commit・push・配信する）。**
`npm run verify`（typecheck・test 966件・build:all・e2e・e2e:update）が
全て成功済み。VERSIONは`forge-v42`に更新しビルド済み。

**経緯（今回）**: 本人から分析画面の6フェーズにわたる再設計指示書（刺激/疲労
コストの多次元可視化・4分類疲労内訳・ケイデンス等の反復間動作解析を含む）が
提示されたが、調査の結果(a)提案の大半（同一処方比較・レース通過分析・CFE推移・
ACWR・転移度バランス・カバレッジ）は既に画面に存在し再設計というより再編成、
(b)刺激/疲労コストの可視化は直前のセッションで見送った刺激ベース生成エンジンに
依存しておりそのままでは実装不可、(c)ケイデンス・ストライド等の反復間動作データは
FITパース含めコードベースのどこにも存在せず新規データ収集無しには実装不可能、
(d)提案の4分類疲労内訳は既存`computeReadiness`の6要素内訳より根拠が薄い——
という理由で全体実装を推奨しないと回答。その上で新規データ不要・低リスクな
2点だけを実装する方針にした。

**実装内容**:
1. `src/lib/core/timeline.ts`（新規）: 負荷/ACWR・睡眠・脚疲労・張り・
   安静時心拍・シグナル（黄/赤）・休養日・レース日を28日間の同じ日付軸に
   統合する`buildTimeline`。新しい推定ロジックは持たず、既存データ
   （`loadSeries`・`DailyCheck`・`Session.category==="off"`・
   `FitnessMarker.type==="race"`）をそのまま並べ直すだけ。
   `/api/analysis`・`pwa/api-shim.ts`双方に`timeline`フィールドとして追加、
   `app/analysis/page.tsx`に`TimelineCard`（SVG複合チャート）を新設。
2. `src/lib/core/samePrescription.ts`の`PrescriptionOccurrence`に
   `nextDayLegs`を追加（`rpe`は既存）。`app/analysis/page.tsx`の
   同一処方比較表にRPE・翌日脚の重さの列を追加（データは既存、
   表示していなかっただけ）。

**見つけて直したバグ**: `TimelineCard`のACWR/安静時HR折れ線パス生成が、
元配列のindexで`M`/`L`コマンドを決めていたため、**先頭の日にデータが無いと
不正なSVGパス**（`L`から始まる）になっていた。E2Eの実データ（睡眠等が
未記録の日を含む）で実際にコンソールエラーとして検出。`sparsePath`
ヘルパーに置き換え、有効な点だけを見て最初の点を`M`にするよう修正。

**検証**: `tests/timeline.test.ts`（6件）・`tests/samePrescription.test.ts`
への追加2件、いずれも先に実装を壊して赤を確認してから復元（T-4）。
パスのバグはE2Eで先に赤（コンソールエラー）→ 修正 → 緑を確認。

---

**前回（forge-v41・2026-07-31・刺激ベース生成エンジンの大規模提案を検討し、
最小代替案を実装）の経緯**: 本人から「刺激ベース・トレーニング生成エンジン」への大規模移行
...
1. README.mdの「セキュリティ・プライバシー監査」節末尾「指摘したが今回直して
   ...
   扱ってよい
2. ジョグ＋坂ダッシュのような複合メニューを本当に1枠で記録したい場合、
   データモデル（Session/SessionResultへのsegments概念の追加）の設計判断が
   必要（今回は警告表示のみで対応し、モデル変更は見送った）
3. 刺激ベース生成エンジンの本格導入は今回見送ったが、指示書自体は会話ログに
   残っている。個人反応データが十分に蓄積された将来、再検討の余地はある
4. 「分析・データ可視化機能の再設計」提案のうち、刺激/疲労コスト可視化
   （フェーズ4）とケイデンス等の反復間動作解析（フェーズ5）は上記と同じ理由で
   今回見送った。前者は刺激ベース生成エンジン、後者は新規データ収集
   （手入力またはFIT拡張）が前提になる
...
| 項目 | 値 |
| --- | --- |
| ソース `pwa/sw.js` | `forge-v42`（未コミットのローカル変更） |
| `pwa-dist/sw.js` | `forge-v42`（ローカルビルド済み・未配信） |
| 公開中（`gh-pages`） | `forge-v41`（配信済み・2026-07-31） |
