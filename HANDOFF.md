...
```bash
node pwa/visual.mjs --only=today
```

時刻を `2026-08-15`（レースまで41日＝リファレンスと同条件）に固定し、
プランを `2026-07-27` から生成、基準日より前に実施記録を入れてから撮る。
固定しないと毎回違う絵が出て比較にならない。出力は `visual/current/`（gitignore）。
...
Phase 3の分析（PERFORMANCEセグメント新設・期間集計）／ Phase 4 AIメニュー・
記録サマリー（新規画面）／ Phase 5 セッション実行 ／ Phase 6 スプラッシュ・アイコン ／
Phase 7 baseline確定。**まだ push・配信はしていない**（6画面が旧デザインのままで、
中途半端な状態を実機に送らないため）。
