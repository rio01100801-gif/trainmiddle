...
describe("F-2 実際の日誌がそのまま通ること", () => {
  /**
   * 実データで人が選ばないといけないのは1行だけ（forge-v84時点）。
   *
   * 7/18 の `1000(3:15-25)×4` はGRPの137%。ここはCVと閾値のどちらもありうる帯で、
   * **設定タイムだけでは決まらない**（CVと閾値の設定は実測LTから作られており、
   * 互いに6〜8秒/kmしか離れていない）。
   *
   * 以前はここを距離だけで「CV」と断定していた。1:49の走者にとって
   * 1000mを3:15〜3:25は閾値の側なので、断定は**間違っていた**。
   * 黙って間違えるより、選んでもらうほうがいい。
   *
   * この件数が増えたらここが落ちる。増えるなら判定を見直すこと
   * （貼るたびに何行も選ばせるのでは、入力の負担を減らす目的を損なう）。
   */
  it("実データで選択が要るのは1行だけ（CVと閾値の帯）", () => {
    const notReady = rows.filter((r) => !r.ready);
    expect(notReady.map((r) => r.date)).toEqual(["2026-07-18"]);
    expect(notReady[0].issues.join("/")).toContain("CVと閾値");
  });

  it("複合（300＋600＋600）は断定できる（反復であって分割ではない）", () => {
    const r = byDate("2026-07-06");
    expect(r.categoryUncertain).toBeFalsy();
