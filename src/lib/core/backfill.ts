...
export function assessCurrentFitness(
  ...
  for (const e of entries) {
    ...
    if (!imp) {
      excluded.push({
         ...
         * 落ちた理由を取り違えないこと。
         *
         * 以前はまとめて「カテゴリ・距離・タイムのいずれかが不足」と出していたが、
         * CVや閾値は**入力が揃っていても**落ちる（800m相当への換算比率が無いため）。
         * 「入れ方が悪い」と読めてしまい、何を足せば測れるようになるのか分からない。
         */
        reason:
  ...
  if (used.length === 0) {
    return {
       ...
       * 何を足せば測れるようになるかまで書く。
       *
       * 「実測がありません」だけだと、ジョグを何十件入れても状況が変わらない
       * ことが伝わらない（実際、ジョグ22件とCV1件で0件になった例がある）。
       * 使える種類を名指しする。
       */
      notes: [
        ...notes,
        "現在地を推定できる実測がありません。" +
          "使えるのは 800m・600m・400m のレース／タイムトライアルか、" +
          "高乳酸・モデリング・経済走のインターバル（実施タイム入り）です。" +
          "ジョグ・持続走とCV・閾値は、負荷とLT推定には入りますが800m能力の推定には使えません。",
      ],
    };
  }
