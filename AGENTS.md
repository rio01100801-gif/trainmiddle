...
ここには作業のときに必要なことだけ書く。文書の役割を分けてある。

| 文書 | 役割 |
| --- | --- |
| `README.md` | なぜその設計にしたか |
| `HANDOFF.md` | いまどうなっているか |
| `BACKLOG.md` | 何が残っているか（**未完了はここだけを正とする**） |
| `CHANGELOG.md` | 何をやってきたか |
| `OPERATIONS.md` | 障害対応・リリース手順・ロールバック |

**件数や版数を文書に手で書かない。** テスト件数は `npm test` の出力、
配信中の版は `pwa/sw.js` の `VERSION` が唯一の正。
`npm run ci:docs` が手書きを禁止している（`CHANGELOG.md` だけは当時の記録なので例外）。
...
| ユニットテスト | `npm test`（件数は出力を見る。文書に書かない） |
...
**`node -e` で正規表現を書かない。** シェルを1回通るので `d` `s` `` が黙って消え、
`/^s*/` が `/^s*/` になる。**置換したつもりで何も置換していない**状態が生まれ、
そのまま「壊して確認した」と報告することになる。
スクリプトはファイルに書いて `node script.mjs` で走らせ、
置換したかどうかを `replaced: true` として必ず出す。
